    ...
    public sealed class MainForm : Form
    {
        ...
        private GroupBox CreateEightBallPanel()
        {
            ...
            _eightBallButton.Click += (_, _) =>
            {
                ...
                int index = (int)(raw / (LcgRandom.M / (ulong)EightBallAnswers.Length));
                if (index >= EightBallAnswers.Length)
                    index = EightBallAnswers.Length - 1;

                _eightBallResultLabel.Text = EightBallAnswers[index];
                _lastRangeLabel.Text = $"raw = {raw} → индекс = {index} из {EightBallAnswers.Length - 1}";
                UpdateCurrentState();
            };
