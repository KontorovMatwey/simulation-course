...
namespace RandomEventsGui
{
    ...
    public sealed class MainForm : Form
    {
        ...
        private struct EightBallItem
        {
            ...
            public EightBallItem(string text, int weight)
            {
                Text = text;
                Weight = weight;
            }
        ...
        {
            new("Без сомнений — да.", 10),
            new("Да.", 15),
            new("Скорее да.", 12),
            new("Похоже, что да.", 10),
            new("Ответ туманный, но шанс есть.", 8),
            new("Спроси позже.", 6),
            new("Сейчас не время.", 5),
            new("Скорее нет.", 8),
            new("Нет.", 10),
            new("Точно нет.", 6),
            new("Очень маловероятно.", 4),
            new("Знаки говорят: да.", 6),
            new("Знаки говорят: нет.", 6),
            new("Да, но с подвохом.", 4),
            new("Нет, и это лучше принять.", 3),
            new("Возможно.", 7),
        };
        ...
        private GroupBox CreateEightBallPanel()
        {
            ...
            {
                ulong raw = _lcg.NextRaw();
                
                int totalWeight = 0;
                foreach (var item in EightBallAnswers)
                    totalWeight += item.Weight;

                int value = (int)(raw % (ulong)totalWeight);
                ...
                foreach (var item in EightBallAnswers)
                {
                    ...
                    if (value < cumulative)
                    {
                        ...
                        _lastRangeLabel.Text = $"raw = {raw} → value = {value} / {totalWeight}";
                        break;
                    }
                }
