...
namespace WeatherMarkovModelWinForms
{
    ...
    public sealed class MainForm : Form
    {
        ...
        private void BuildTopPanel(Panel topPanel)
        {
            ...
            _durationCombo.Items.AddRange(new object[] { 15, 30, 90, 365 });
            ...
            infoLabel.Text = "Вне диагонали вводятся интенсивности переходов qij >= 0. Диагональ считается автоматически. Значения больше 1 допустимы — это интенсивности, не вероятности.";
        ...
        private void BuildLeftPanel(Panel leftPanel)
        {
            ...
            timelineGroup.Text = "Визуализация по дням";
        ...
        {
            // Ничего критичного тут не нужно, но форма уже полностью создана.
            // Можно спокойно сделать первый redraw без риска проблем с размерами.
        ...
        private void StartSimulation()
        {
            ...
            _targetDays = (int)_durationCombo.SelectedItem;
            int monthIndex = _monthCombo.SelectedIndex + 1;
            _startDate = new DateTime(DateTime.Now.Year, monthIndex, 1);
            _currentState = 1;
            _currentDayIndex = 0;
            _running = true;
            ...
            UpdateCurrentLabel("Старт: " + _stateNames[_currentState] + " | " + _startDate.ToString("MMMM yyyy", CultureInfo.CurrentCulture));
        ...
        private void Timer_Tick(object sender, EventArgs e)
        {
            ...
            if (_currentDayIndex >= _targetDays)
            {
                FinishSimulation();
                return;
            }

            DateTime date = _startDate.AddDays(_currentDayIndex);
            _dailyStates.Add(_currentState);
            _dailyDates.Add(date);
            _counts[_currentState]++;

            _currentDayIndex++;
            UpdateCurrentLabel(string.Format(CultureInfo.CurrentCulture, "День {0}/{1}: {2} ({3:dd.MM.yyyy})", _currentDayIndex, _targetDays, _stateNames[_currentState], date));
            _timelinePanel.SetData(_dailyStates, _dailyDates, _currentDayIndex, true);
            _timelinePanel.Invalidate();
        ...
        private void FinishSimulation()
        {
            ...
            for (int i = 1; i <= StateCount; i++)
                _empirical[i] = _counts[i] / (double)days;
        ...
        private void BuildReportText(int days)
        {
            StringBuilder sb = new StringBuilder();
            sb.AppendLine("Марковская модель погоды");
            sb.AppendLine();
            sb.AppendLine("Стартовый месяц: " + _startDate.ToString("MMMM yyyy", CultureInfo.CurrentCulture));
        ...
        private void WriteCsvFiles(int days)
        {
            string outDir = Path.Combine(AppContext.BaseDirectory, "output");
            Directory.CreateDirectory(outDir);

            string stamp = DateTime.Now.ToString("yyyyMMdd_HHmmss");
            _dailyCsvPath = Path.Combine(outDir, "weather_daily_" + stamp + ".csv");
            ...
            dailyLines.Add("day,date,state_id,state_name");
            for (int i = 0; i < _dailyStates.Count; i++)
            {
                dailyLines.Add(string.Format(CultureInfo.InvariantCulture, "{0},{1:yyyy-MM-dd},{2},{3}", i + 1, _dailyDates[i], _dailyStates[i], _stateNames[_dailyStates[i]]));
            }
            File.WriteAllLines(_dailyCsvPath, dailyLines.ToArray());

            List<string> summaryLines = new List<string>();
            summaryLines.Add("metric,state,values");
            summaryLines.Add(string.Format(CultureInfo.InvariantCulture, "days,,{0}", days));
            summaryLines.Add(string.Format(CultureInfo.InvariantCulture, "start_month,,{0:yyyy-MM}", _startDate));
            for (int i = 1; i <= StateCount; i++)
    ...
    public sealed class TimelinePanel : Panel
    {
        ...
        protected override void OnPaint(PaintEventArgs e)
        {
            ...
            e.Graphics.DrawString("Визуализация по дням", titleFont, Brushes.Black, left, 0);
            ...
            for (int i = 0; i < total; i++)
            {
                ...
                if (i < _dates.Count)
                    e.Graphics.DrawString(_dates[i].ToString("dd.MM", CultureInfo.InvariantCulture), dateFont, Brushes.Black, x + 8, y + 48);
        ...
        private static Color GetColor(int state)
        {
            switch (state)
            {
                ...
                case 5: return Color.LightCoral;
    ...
    public sealed class DistributionPanel : Panel
    {
        ...
        public void SetData(double[] empirical, double[] stationary, int days)
        {
